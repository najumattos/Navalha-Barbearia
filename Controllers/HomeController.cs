    ...
    IUsuarioContextoService usuarioContextoService) : Controller
{
    ...
    public IActionResult ConfirmarAgendamento(HomeResumoAgendamentoViewModel viewModel)
    {
        try
        {
            ...
            var agendamentoCriado = agendamentoService.Criar(agendamentoParaCriar);
    ...
    {
        var agendamentoAtual = agendamentoService.ObterPorId(idAgendamento, 0, TipoAcessoEnum.Administrador);
        if (agendamentoAtual is null)
        {
            return NotFound();
        }

        var cliente = clienteService.ObterPorCpfPublico(agendamentoAtual.Cliente.CPF) ?? agendamentoAtual.Cliente;
        var historicoRecente = agendamentoService.ObterPorCpfCliente(cliente.CPF, TipoAcessoEnum.Cliente)
    ...
    private HomeResumoAgendamentoViewModel CriarResumoAgendamentoViewModel(AgendamentoModel agendamentoAtual, bool exibirBotaoConfirmar)
    {
        ...
        var slotSelecionado = slotHorarioService.ObterPorId(agendamentoAtual.SlotHorarioId)
            ?? throw new KeyNotFoundException($"Slot de horario {agendamentoAtual.SlotHorarioId} nao encontrado.");

        var barbeiro = barbeiroService.ObterPorId(agendamentoAtual.Barbeiro.Id)
        ...
        var cliente = clienteService.ObterPorCpfPublico(agendamentoAtual.Cliente.CPF)
    ...
    public IActionResult HomeAdministrador(int? idBarbeiro)
    {
        ...
        var idBarbeiroResolvido = idBarbeiro ?? usuarioContextoService.ObterIdBarbeiro();
        if (!idBarbeiroResolvido.HasValue)
        {
            return RedirectToAction("Login", "Auth");
        }

        var login = loginService.ObterPorBarbeiroId(idBarbeiroResolvido.Value);
        ...
        var barbeiros = barbeiroService.ObterTodos();
        var agendamentos = agendamentoService.ObterTodosParaAdministrador(TipoAcessoEnum.Administrador);
    ...
    public IActionResult HomeFuncionario(int? idBarbeiro)
    {
        ...
        var funcionario = barbeiroService.ObterPorId(idBarbeiroResolvido.Value);
    ...
    public IActionResult AtualizarPrecoPorBarbeiroFuncionario(int procedimentoId, decimal precoPorBarbeiro)
    {
        ...
        var login = loginService.ObterPorBarbeiroId(idBarbeiro.Value);
        if (login?.TipoAcessoEnum != TipoAcessoEnum.Funcionario)
        {
            return Forbid();
        }
        ...
        try
        {
            barbeiroService.AtualizarPrecoPorBarbeiro(idBarbeiro.Value, procedimentoId, precoPorBarbeiro, TipoAcessoEnum.Funcionario);
    ...
    public IActionResult BuscarClientePorCpf(string cpf)
    {
        ...
        var cliente = clienteService.ObterPorCpfPublico(cpf);
        if (cliente is null)
        {
    ...
    {
        var login = loginService.ObterPorBarbeiroId(idBarbeiro);
        if (login?.TipoAcessoEnum != TipoAcessoEnum.Funcionario)
        {
            return Forbid();
        }
        ...
        try
        {
            ...
            agendamentoService.AtualizarDoFuncionario(idAgendamento, idBarbeiro, agendamento, TipoAcessoEnum.Funcionario);
    ...
    {
        var funcionario = barbeiroService.ObterPorId(idBarbeiro)
        ...
        return new HomeFuncionarioViewModel
        {
            ...
            Agendamentos = agendamentoService.ObterPorBarbeiroId(idBarbeiro, TipoAcessoEnum.Funcionario),
            Clientes = clienteService.ObterPorBarbeiro(idBarbeiro, TipoAcessoEnum.Funcionario)
        };
    }
