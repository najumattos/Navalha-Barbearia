...
public class ProcedimentosController(IProcedimentoService procedimentoService, IBarbeiroService barbeiroService, IUsuarioContextoService usuarioContextoService) : Controller
{
    public IActionResult Index()
    {
        ...
        if (exibirColunasFuncionario)
        {
            ...
            {
                var barbeiro = barbeiroService.ObterPorId(idBarbeiro.Value);
        ...
        {
            Procedimentos = procedimentoService.ObterTodos(),
    ...
    public IActionResult AtualizarAtivoFuncionario(int procedimentoId, bool ativo)
    {
        ...
        try
        {
            if (ativo)
            {
                barbeiroService.AdicionarProcedimentoAoBarbeiro(idBarbeiro.Value, procedimentoId, TipoAcessoEnum.Funcionario);
            }
            else
            {
                barbeiroService.RemoverProcedimentoDoBarbeiro(idBarbeiro.Value, procedimentoId, TipoAcessoEnum.Funcionario);
            }
        }
    ...
    {
        if (usuarioContextoService.ObterTipoAcesso() == TipoAcessoEnum.Cliente)
        {
            return Forbid();
        }

        var procedimento = procedimentoService.ObterPorId(id);
    ...
    public IActionResult Create(ProcedimentoModel procedimento)
    {
        ...
        procedimentoService.Criar(procedimento, TipoAcessoEnum.Administrador);
        return RedirectToAction(nameof(Index));
    }
    ...
    public IActionResult Edit(ProcedimentoModel procedimento)
    {
        ...
        procedimentoService.AtualizarCatalogo(procedimento.Id, procedimento, TipoAcessoEnum.Administrador);
        return RedirectToAction(nameof(Index));
    }
    ...
    public IActionResult DeleteConfirmed(int id)
    {
        ...
        procedimentoService.Excluir(id, TipoAcessoEnum.Administrador);
        return RedirectToAction(nameof(Index));
    }
